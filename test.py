...
# train_data = data.iloc[:sample,:]
# test_data = data.iloc[sample:,:]
...
data["label"] = data["Daily average Alipay Wallet balance"]/max(data["Daily average Alipay Wallet balance"]) + data['Security funds in Taobao']/max(data['Security funds in Taobao']) + data['Daily payment activity']/max(data['Daily payment activity']) + data["Daily average YueBao balance"]/max(data["Daily average YueBao balance"])
data.sort_values(by=label, ascending=False,inplace=True)
data.reset_index(inplace=True)
data["label"] = 1
data.loc[:45000,["label"]] = 0
...
def mono_bin(Y, X, n=10):
    ...
    d3['total'] = d2.count().Y
    d3['rate'] = d2.mean().Y
    d3['woe']=np.clip(np.log((d3['rate']/(1-d3['rate']))/(good/bad)),-999,+999)
...
def self_bin(Y,X,cut):
    ...
    d3['total'] = d2.count().Y
    d3['rate'] = d2.mean().Y
    d3['woe'] = np.clip(np.log((d3['bad']/badnum)/((d3['total'] - d3['bad'])/goodnum)),-999,+999)
...
plt.figure()
cut = [ninf] + list(np.arange(45, 120, 5)) + [pinf]
d1 = pd.DataFrame({"Score": scoretable1["Score"], "Y": scoretable1[label], "Bucket": pd.cut(scoretable1["Score"], cut)})
d2 = d1.groupby('Bucket', as_index=True)

plt.bar([35] + list(np.arange(45, 120, 5)), list(d2.count().Y/max(d2.count().Y)),width=3,label='Number of Users')
plt.plot([35] + list(np.arange(45, 120, 5)), list(d2.sum().Y/d2.count().Y),color = 'red',label='Bad Users Rate')
...
# print('Accuracy: ',sum([1 if i == j else 0 for i,j in zip(Y_test,Y_pred)])/len(Y_pred))
plt.show()
